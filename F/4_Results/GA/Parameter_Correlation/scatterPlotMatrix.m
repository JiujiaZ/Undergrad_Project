function H=scatterPlotMatrix(X, var_labels, params,var_types, need_save)
% Scatter Plot Matrix
    ...
    for j = 1:i
        ...
        if i == j
            ...
            text(0.5, 0.5, var_labels(j), 'Units', 'normalized',...
                'HorizontalAlignment', 'center', 'Color', 'black', 'FontSize', 20,'FontWeight', 'bold', 'Interpreter','Latex')
        else
            % Scatter plots in the lower triangular
            scatter(X(:, j), X(:, i), 50, 'b', '.')
            % add nominal value
            hold
            scatter(params(j),params(i),200,'x','r','LineWidth',2)
...
for i = 1:p
    for j = (i+1):p
        ...
        text(0.5, 0.5, sprintf('R=%.2f', r), 'Units', 'normalized',...
            'HorizontalAlignment', 'center', 'Color', 'black', 'FontSize', 14,'FontWeight', 'bold') 
...
hp_t = get(subplot(9, 9, 9),'Position')
hp_b = get(subplot(9, 9, 72),'Position')

c=colorbar;
c.Position=[hp_b(1)+0.075 hp_b(2)  0.01  hp_t(2)+hp_t(4)-hp_b(2)];
c.YTick=[0:0.25:1];

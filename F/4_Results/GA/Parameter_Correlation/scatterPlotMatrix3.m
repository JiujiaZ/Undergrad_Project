function H=scatterPlotMatrix3(X, var_labels, params,var_types, need_save)
% Scatter Plot Matrix
...
BigH=figure('position', [0, 0, 560*10, 420*10])
    ...
    for j = 1:i
        ...
        if i == j
            if strcmp(var_types{i}, 'discrete')
                %hist(X(:, i))
                histogram(X(:, i),'Normalization','probability','FaceColor',[0, 0.45, 0.74],'EdgeColor',[0, 0.45, 0.74],'FaceAlpha',1)
            ...
            text(0.5, 0.5, var_labels(j), 'Units', 'normalized',...
                'HorizontalAlignment', 'center', 'Color', 'black', 'FontSize', 40,'FontWeight', 'bold', 'Interpreter','Latex')
        else
            % Scatter plots in the lower triangular
            scatter(X(:, j), X(:, i), 200, 'b', '.')
            % add nominal value
            hold
            scatter(params(j),params(i),700,'x','r','LineWidth',4)
...
for i = 1:p
    for j = (i+1):p
        ...
        text(0.5, 0.5, sprintf('R=%.2f', r), 'Units', 'normalized',...
            'HorizontalAlignment', 'center', 'FontWeight', 'bold','FontSize',35) 
...
hp_t = get(subplot(3, 3, 3),'Position')
hp_b = get(subplot(3, 3, 6),'Position')

c=colorbar;
c.Position=[hp_b(1)+0.23 hp_b(2)  0.01  hp_t(2)+hp_t(4)-hp_b(2)];
c.YTick=[0:0.25:1];
c.YTickLabel={'-1','-0.5','0','0.5','1'};
c.Label.String='R'
c.Label.FontSize=25
c.Label.Position=[2.9,0.62,0];
c.Label.Rotation=0;
c.FontSize=20;
colormap(cmap)
if need_save
